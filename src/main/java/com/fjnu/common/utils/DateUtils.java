...
public class DateUtils {
    ...
    /**
     * 将ISO 8601格式的时间字符串转换为java.sql.Timestamp格式日期,用于数据库保存
     *
     * @param strDate 符合 yyyy-MM-dd'T'HH:mm:ss'Z'的日期的字符串
     * @return java.sql.Timestamp类型日期对象（如果转换失败则返回null）
     */
    public static java.sql.Timestamp strISO8601ToSqlDate(String strDate) throws ParseException {
    ...
    public static java.sql.Timestamp lazadaResponseDateStrToSqlDate(String strDate) throws ParseException {
        SimpleDateFormat sf = new SimpleDateFormat("yyyy-MM-dd HH:mm:ss Z", Locale.CHINA);
        java.util.Date date = sf.parse(strDate);
        java.sql.Timestamp dateSQL = new java.sql.Timestamp(date.getTime());
        return dateSQL;
    }
     ...
     * @return
     */
    public static String dateToISO8601DateFormatStr(long time) {
        DateFormat df = new SimpleDateFormat("yyyy-MM-dd'T'HH:mm:ss'Z'");
        String str = df.format(new java.util.Date(time));
        return str;
    }
